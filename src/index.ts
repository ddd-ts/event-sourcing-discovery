...
export const Snapshottable = <const S extends { [key: string]: Shorthand }>(
  shape: S,
) => Subtrait([EventSourced([])],(base) => Dict(shape, class extends base {
  static fromSnapshot<TH extends Constructor>(this: TH, snapshot: any & { revision: number }): InstanceType<TH> {
    const instance = (this as any).deserialize(snapshot) as any as ImplementsTrait<ReturnType<typeof EventSourced>>;
...
export const EventSourced = <Events extends Constructor<IEsEvent>[]>(events: Events) =>
  Trait((base) => {
...
class AccountStoreWithSnapshots {
  ...
  load(id: AccountId) {
    ...
    if (snapshot) {
      ...
      const hasChanges = stream.some((event) => event.revision > snapshot.revision);
...
class Transaction extends Derive(EventSourced([TransactionInitiated]), Snapshottable({
  id: TransactionId,
  from: AccountId,
  to: AccountId,
  amount: Number,
  ...
  static initiate(from: AccountId, to: AccountId, amount: number) {
    return Transaction.new(TransactionInitiated.new(TransactionId.generate(), { from, to, amount }));
  }
...
class AccountCashflowProjection {
  ...
  handle(event: IEsEvent, trx: Transaction) {
    if(!(event instanceof AccountOpened || event instanceof Deposited || event instanceof Withdrawn)){
  ...
  async initialize(accountId: AccountId, trx: Transaction) {
    ...
    const cashflow = new AccountCashflow({ accountId, cashflow: account.balance });
...
abstract class ProjectionCheckpointStore {
  ...
  abstract save(projection: any, checkpoint: ProjectionCheckpoint, trx?: Transaction): void;
}
...
class AccountCashflowProjector {
  ...
  async start() {
    const checkpoint = await this.projection.checkpointStore.load(this.projection);
   ...
   * only start sending updates once the projection has handled the first event that occured after
  ...
  private async rebuild(): Promise<void> {
    ...
    const checkpoint = new ProjectionCheckpoint(this.projection.name, this.projection.stream, 0);
  ...
  private async continue(checkpoint: ProjectionCheckpoint): Promise<void> {
    for await (const event of this.eventStore.follow(this.projection.stream, checkpoint.revision)) {
      await this.projection.transaction.perform(async (trx) => {
        const checkpoint = await this.projection.checkpointStore.load(this.projection, trx)
...
class AddAccountToRegistrySaga extends Shape({
  ...
  static onAccountOpened(event: AccountOpened) {
    return new this({ id: SagaId.generate(), accountId: event.aggregateId, finished: false });
  }
...
class AddAccountToRegistrySagaOrchestrator {
  ...
  async start() {
    ...
    const checkpoint = await this.sagaManager.checkpointStore.load(this.sagaManager.constructor.name);
    ...
    if(checkpoint.stream !== this.sagaManager.stream){
      ...
      const event = this.eventStore.findEvent(this.sagaManager.stream, checkpoint.lastEventId);
      checkpoint.revision = event.revision;

      await this.sagaManager.checkpointStore.save(checkpoint);
    }

    for await (const event of this.eventStore.follow(this.sagaManager.stream, checkpoint.revision)) {
